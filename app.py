...
def postgres_messages(method, user_id_to=None, user_id_from=None, text=None):
    psql = psycopg2.connect(host=os.environ['POST_HOST'], user=os.environ['POST_USER'], password=os.environ['POST_PASSWORD'], database=os.environ['POST_DB'])
    try: 
        psql.autocommit = True
        cursor = psql.cursor()
        ...
        cursor.execute(
            """CREATE TABLE IF NOT EXISTS message(id SERIAL PRIMARY KEY, user_id_from BIGINT NOT NULL, user_id_to BIGINT NOT NULL, text TEXT);"""
        )


        if method == 'send_text':
            cursor.execute(
                """INSERT INTO message(user_id_from, user_id_to, text) VALUES(%s, %s, %s);""", (user_id_from, user_id_to, text,)
            )
...
def index():
    if validate_user() != None:
        ...
        return render_template('index.html', users=postgres_profiles('find_all_users'))
